...
# เวลาที่โปรเซสนี้เริ่ม — Render ฟรีหลับเองแล้วบูตใหม่ทุกครั้งที่มีคนเรียก
# เก็บไว้เทียบใน /healthz จะได้รู้ว่า "ตัวปลุก" ปลุกตรงเวลาหรือเปล่า
...
def healthz():
    ...
    up_min = ตื่นมากี่นาทีแล้ว — ไว้ตรวจว่า "ตัวปลุก" ทำงานตรงเวลาไหม
    เปิดดูตอน ๘ โมง ถ้าได้เลขราวๆ ๕๐ แปลว่ารอบ ๗:๑๑ ปลุกสำเร็จ
    ถ้าได้ ๐-๑ แปลว่าเพิ่งตื่นเพราะเราเปิดเอง = ตัวปลุกไม่ทำงาน
    """
    return {"ok": True, "jobs": len(docmode._jobs),
            "up_min": round((datetime.now() - _STARTED).total_seconds() / 60),
...
def api_doc_status(job_id: str, user: str = Depends(current_user)):
    ...
    keys = ("status", "receipt_no", "doc_no", "doc_title", "doc_date", "sender",
            ...
            "total_pages", "sig_page", "redo_no", "book_id", "source", "all_pages")
